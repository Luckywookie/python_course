...
def analys_text(lst):
    for i in range(len(lst)):
        if lst[i].islower() and lst[i + 1].islower():
                s = lst[i:i+2]
                m = lst[i+2:]
                j = 0
                while m[j].isupper():
                    s = s + m[j]
                    j += 1
                    if m[j].islower() and m[j+1].islower():
                        s = s + m[j:j+2]
                        yield s
...
# ======================================================================== #
# прототип первой задачи
l = 'HHHggFFFFFCCCkkkDghghghgFFFFbbbb'
...
n = number[i:i + 6]
m = [int(x) for x in n]
print n, m, reduce(lambda res, y: res * y, m, 1)
