...
# 2016.10.17_08:06:44 checked. prusanov
...
if f == '1':
    try:
        os.mkdir(name)
    except OSError:     # точнее будет - FileExistsError
        print('Такая директория уже существует')
elif f == '0':
    try:
        os.rmdir(name)
    except OSError:     # а здесь тогда - FileNotFoundError
...
# 2016.10.17_08:09:48 checked. prusanov
...
filename = os.path.split(str(sys.argv))     # а почему бы не обратиться к sys.argv[1] ?
